...
void main()
{
	...
	float index = texture2D(VirtualDisplay, Frag_UV).r;
	Out_Color.rgb = texture2D(Palette, vec2(index, 0.5)).rgb;
#else
	Out_Color.rgb = texture2D(VirtualDisplay, Frag_UV).rgb;
#endif

#ifdef ENABLE_BLOOM
	vec3 bloom = texture2D(Bloom, Frag_UV).rgb;
