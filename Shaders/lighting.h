...
float getLightRampValue(float z, float worldAmbient)
{
	...
	float lightSource0 = 31.0 - (texture2D(Colormap, vec2(d0 / 256.0, 0.0)).g*255.0 / 8.23 + worldAmbient);
	float lightSource1 = 31.0 - (texture2D(Colormap, vec2(d1 / 256.0, 0.0)).g*255.0 / 8.23 + worldAmbient);
	...
	float lightSource = 31.0 - (texture2D(Colormap, vec2(depthScaled / 256.0, 0.0)).g*255.0 + worldAmbient);
#endif
	return lightSource;
}
